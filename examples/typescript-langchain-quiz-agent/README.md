...
A quiz generation specialist built with [LangChain.js](https://js.langchain.com/) and bindufied using the [Bindu TypeScript SDK](../../sdks/typescript/). One `bindufy()` call transforms the quiz generator into a full A2A-compliant microservice with DID identity, authentication, x402 payments, and task scheduling.

## What This Example Demonstrates

- Writing a quiz generator in TypeScript using LangChain.js (`ChatOpenAI`)
...
- Generates exactly 10 MCQs with explanations from any provided text
...
Developer runs: npx tsx index.ts
  ...
  │  quiz_generator()    │  :50052       │  Scheduler, Storage          │
  │                      │               │  HTTP Server :3773           │
  │  @bindu/sdk          │               │                              │
  └─────────────────────┘               └──────────────────────────────┘
                                                    ▲
...
- `@langchain/openai` — LangChain.js OpenAI integration (works with OpenRouter)
...
curl -s -X POST http://localhost:3773 \
  ...
  -d '{
    ...
    "params": {
      "message": {
        "role": "user",
        "parts": [{"kind": "text", "text": "Generate a quiz from this text: Photosynthesis is the process by which plants convert sunlight into energy. They use chlorophyll to capture light energy and convert it into chemical energy stored in glucose."}],
...
// Create LangChain model with OpenRouter
...
bindufy(
  ...
  async (messages: ChatMessage[]) => {
    // Extract latest user input and construct quiz prompt
...
9. LangChain calls OpenRouter OpenAI API
...
```
typescript-langchain-quiz-agent/
  quiz-agent.ts               # Agent code — LangChain.js + bindufy()
  ...
  skills/
    quiz-generation/
      skill.yaml              # Quiz skill definition (YAML format)
      SKILL.md                # Quiz skill documentation (Markdown format)
    research/
      skill.yaml              # Research skill definition (YAML format)
      SKILL.md                # Research skill documentation (Markdown format)
```
...
- Try the [TypeScript LangChain Research Agent](../typescript-langchain-agent/) for a research-focused example
